...
class PointsOfClassifiedsTeams extends Component {
  ...
  renderFinalResult = () => {
    ...
    points +=
      ...
      this.numberPointsKnockoutMatches(this.props.group).first;
    points +=
      structuredReturn.second.type *
      this.numberPointsKnockoutMatches(this.props.group).second;
    points +=
      structuredReturn.third.type *
      this.numberPointsKnockoutMatches(this.props.group).third;
    points +=
      structuredReturn.fourth.type *
      this.numberPointsKnockoutMatches(this.props.group).fourth;
  ...
  renderTopScorer = () => {
    ...
    return (
      <TopScorer
        name={userFinalResult.nameOfTopScorer}
        goals={userFinalResult.goals}
        points={points}
    ...
    for (let index = 0; index < teamsOfUser.length; index++) {
      ...
      if (teamsOutcome.includes(element)) {
        structuredReturn.points +=
          this.numberPointsKnockoutMatches(group).classified;
        if (teamsSpecificPosition.includes(element)) {
          structuredReturn.teams.push({ id: element, type: "specific" });
          structuredReturn.points +=
            this.numberPointsKnockoutMatches(group).specificTeam;
        } else {
