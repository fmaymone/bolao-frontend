...
class PointsOfMatch extends Component {
  ...
  calculateRegularPoints = (userMatch, outcomeMatch) =>{
    ...
    if(Math.sign(userMatch.home_result - userMatch.away_result) === Math.sign(outcomeMatch.home_result - outcomeMatch.away_result)){
      console.log('oi');
      //draw
  ...
  calculateKnockoutPoints= (userMatch, outcomeMatch) =>{
    let result = 0;
    let pointsPerTeam = this.numberPointsKnockoutMatches(userMatch.group).specificTeam;
